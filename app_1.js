...
function initGraph(HTMLContainer) {
  ...
  // 실제적인 컨텐츠 컨테이너
  const stageChart = stageChartWrapper
    .append("g")
  ...
  const brush = d3.brush().extent([
    [0, 0],
    [config._width, config._height],
  ]); // 设定最大大小，其实已经受到左图 zoom extent 的限制了
  ...
  function onBrush() {
    ...
    if (Array.isArray(d3.event.selection)) {
      ...
      stageChartWrapper.attr(
        "transform",
        `translate(${scaleX(-brushX)}, ${scaleY(-brushY)}) scale(${zoomScale})`
      );
    }
  }
  ...
  function onZoom() {
    ...
    if (d3.event.sourceEvent && d3.event.sourceEvent.type === "brush")
      // 如果把这一行注释掉，在拖动 brush 的时候也会有 zoom 事件，即：event.type == zoom, event.sourceEvent.type == brush
      return null;
  ...
  // 미니맵 브러쉬 초기화
  stageBrush.call(brush).call(brush.move, [
    // 设置 brush 的初始大小
    [0, 0],
